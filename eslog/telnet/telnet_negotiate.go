...
func (t *Ctx) negotiate(cmd Command, code byte) (err error) {
	s := t.os.load(code)
	fmt.Printf("Received %s option %s", cmd, s.opt)
	...
	case will:
		...
		case nsNo:
			if s.opt.LetHim() {
				...
				fmt.Printf("Option %s enabled for him", s.opt)
				s.opt.SetHim(t, true)
			} else {
		...
		case nsWantNo:
			...
			fmt.Printf("Option %s disabled for him", s.opt)
	...
	case do:
		...
		case nsNo:
			if s.opt.LetUs() {
				...
				fmt.Printf("Option %s enabled for us", s.opt)
				s.opt.SetUs(t, true)
			} else {
		...
		case nsWantNo:
			...
			fmt.Printf("Option %s disabled for us", s.opt)
			s.opt.SetUs(t, false)
		case nsWantNoOpp:
			err = fmt.Errorf("%s option %s answered answered by %s", wont, s.opt, do)
			fallthrough
...
func (t *Ctx) subnegotiate(code byte, params []byte) {
	s := t.os.load(code)
	fmt.Printf("Subnegotation option %s", s.opt)
